...
export class EventController  extends ResponseController {
  ...
  async create(@Body() createEventDto: CreateEventDto, @GetCurrentUserId() userId: string): Promise<IResponseWithData> {
    const response_data = await this.eventService.create(createEventDto, userId);
  ...
  async update(@Param('id') id: string,  @GetCurrentUserId() userId: string, @Body() updateEventDto: UpdateEventDto): Promise<IResponseWithData> {
    const response_data = await this.eventService.update(id, userId, updateEventDto);
    return this.responseWithData(response_data);
  }
