...
import { CreateUserDto } from '@app/v1/REST/dto/user/create-user.dto';
import { VerifyTokenDto } from '@app/v1/REST/dto/auth/verify-token.dto';
import { ResendTokenDto } from '@app/v1/REST/dto/auth/resed-token.dto';
import { ForgotPasswordDto } from '@app/v1/REST/dto/auth/forgot-password.dto';
import { ResetPasswordTokenDto } from '@app/v1/REST/dto/auth/reset-password-token.dto';
import { AuthService } from '@app/v1/REST/services/auth/auth.service';
...
export class AuthController extends ResponseController {
  ...
  async login(@Body() signInDto: SignInDto): Promise<IResponseWithData> {
    ...
    return this.responseWithData({
      ...
      type: 'Bearer',
      'access-token-life-span': this.configService.get<string>(
        'JWT_AUTH_TOKEN_EXPIRATION',
      ),
      'refresh-token-life-span': this.configService.get<string>(
  ...
  async updateMyProfile(
    ...
    const response_data = await this.authService.updateMyProfile(
      updateUserDto,
      userId,
    );
    return this.responseWithData(response_data);
